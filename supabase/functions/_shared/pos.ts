...
/**
 * Список заказов, которым пора авто-ретрай (для крона раз в минуту).
 * Берём failed+auto_retry, у кого next_retry_at подошёл: быстрая фаза (раз в минуту)
 * и часовая фаза кодируются в next_retry_at, поэтому один крон обслуживает обе.
 * Предохранители: attempts < RETRY_MAX_TOTAL и возраст заказа < суток.
 */
export async function dueRetryLogIds(supabase: SupabaseClient, limit = 50): Promise<string[]> {
  const nowIso = new Date().toISOString();
  const cutoffIso = new Date(Date.now() - RETRY_MAX_AGE_MS).toISOString();
  const { data } = await supabase.from('iiko_order_log')
    .select('id, next_retry_at')
    .eq('status', 'failed').eq('auto_retry', true).eq('is_test', false)
    .lt('attempts', RETRY_MAX_TOTAL).gt('created_at', cutoffIso)
