...
export function useUserStats() {
   ...
   * Обновление данных.
   *
   * Раньше это безусловно поднимало флаг загрузки — и на главной поверх уже
   * показанного баланса выскакивал скелетон. Человек тянул экран вниз, чтобы
   * освежить цифры, а вместо цифр получал серые прямоугольники на всё время
   * запроса. Теперь состояние загрузки поднимается только когда показывать
   * действительно нечего: есть данные — они и остаются на экране, а новые
   * подставятся, когда приедут.
   *
   * Возвращает промис, чтобы вызывающий (жест «потянуть вниз») мог дождаться
   * завершения, а не считать обновление мгновенным.
   */
  const refetch = useCallback(async () => {
    if (readStatsCache() === null) setIsLoading(true);
    await fetchData();
  }, [fetchData]);
