...
export function PreorderHistoryItem({ preorder, index }: { preorder: Preorder; index: number }) {
  ...
  return (
    <>
      ...
      >
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isCompleted ? 'bg-accent/10' : 'bg-primary/10'}`}>
          {isCompleted ? <Check size={24} className="text-accent" /> : <Coffee size={24} className="text-primary" />}
        </div>
        ...
        <div className="text-right">
          <div className={`text-xs font-medium px-2 py-0.5 rounded-full ${isCompleted ? 'bg-accent/10 text-accent' : 'bg-amber-500/10 text-amber-600'}`}>
            {isCompleted ? 'Выдан' : 'Новый'}
          </div>
      ...
      <Dialog open={showDetail} onOpenChange={setShowDetail}>
        <DialogContent className="max-w-sm mx-auto">
          <div className="flex flex-col items-center text-center space-y-4">
            <div className={`w-14 h-14 rounded-full flex items-center justify-center ${isCompleted ? 'bg-accent/20' : 'bg-primary/10'}`}>
            ...
            <div className={`text-sm font-medium px-3 py-1 rounded-full ${isCompleted ? 'bg-accent/10 text-accent' : 'bg-amber-500/10 text-amber-600'}`}>
