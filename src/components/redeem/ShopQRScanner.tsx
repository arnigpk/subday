...
export function ShopQRScanner({ drinkType, isGuestCoffee, remaining, onShowMyQR, onClose, onRedeemed }: Props) {
  ...
  // равно встанет камера. Раньше здесь мелькал светлый фон с лоадером — та самая
  // белая вспышка.
  //
  // Ни закрашивать этот миг, ни показывать содержимое не нужно: экран забора со
  // своим QR-кодом под ним теперь не рендерится вовсе (см. RedeemPage), так что
  // проступать нечему. Чёрная заливка тут только вредила — она же оставалась на
  // экране в момент успеха и съедала начало анимации.
  const nativeOpening = !webFallback && !nativeFailed;

  return (
    <div className={`fixed inset-0 z-50 flex flex-col overflow-y-auto ${
      nativeOpening ? 'bg-transparent [&>*]:invisible' : 'bg-background'
    }`}>
