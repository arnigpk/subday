...
export default function BaristaHistoryPage() {
  const { isLoading: authLoading } = usePartnerAuth();
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      console.error('Error fetching barista history:', error);
    } finally {
      setIsLoading(false);
    }
  }, [dateFilter, customDateFrom, customDateTo, addressFilter]);
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">
              {dateFilter === 'today' ? 'Сегодня ещё нет списаний' : 'Нет записей за выбранный период'}
            </p>
          </div>
        ) : (
